...
def plot_route(route_geometry, stops):
    # Decode the polyline geometry
    route_points = polyline.decode(route_geometry)

    # Create a map centered on the first stop
    map_center = stops[0]
    route_map = folium.Map(location=[map_center[0], map_center[1]], zoom_start=15)

    # Plot the stops on the map
    for stop in stops:
        folium.Marker([stop[0], stop[1]]).add_to(route_map)

    # Plot the route as a polyline
    folium.PolyLine(route_points, color="blue", weight=5, opacity=0.7).add_to(route_map)

    # Define the output file path
    output_path = "C:/Users/chara/Downloads/optimized_route_map_with_stops.html"  # Change this if needed
...
def main():
    ...
    # If the route was successfully retrieved, plot it
    if route_geometry:
        plot_route(route_geometry, stops)
    else:
